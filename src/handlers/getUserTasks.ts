...
import { getUsersTasksInstance } from '../dal/instances/getUserTasks'
...
const handler: middy.MiddyfiedHandler = middy(async (event: APIGatewayProxyEvent, context: any) => {
    try {
        ...
        if (event.pathParameters?.user_id) {
            const userTaskInstance = await getUsersTasksInstance(cache.db)(event.pathParameters?.user_id)
            
            if (!userTaskInstance) return responseNotFound();

            return responseSuccess(userTaskInstance);
        } else {
